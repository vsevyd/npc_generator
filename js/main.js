...
localStorage.setItem('NPChar',  'Name');
// считывание из Локально Хранилища значения Типа Персонаж и корректирование интерфейса по загрузке страницы
document.addEventListener('DOMContentLoaded', () => {
    let NPChar = localStorage.getItem('NPChar');
    document.getElementById('NP_Char').value = NPChar;
    ShowHideFields(NPChar);
});
// записывание в Локальное Хранилище значения Типа Персонажа и корректирование интерфейса по изменению в селекторе
document.getElementById('NP_Char').addEventListener("change", () => {
    let NPChar = document.getElementById('NP_Char').value;
    localStorage.setItem('NPChar', NPChar);
    ShowHideFields(NPChar);
});
// новый основной алгоритм Генерации
const GeneratedLog = []; // Log of Generated Characters
// добавляем реакцию по клику на кнопку ГЕНЕРИРОВАТЬ
document.getElementById('generate').addEventListener("click", () => {
    // созаём объект Настроек Генерации Персонажа
    const CharacterSettings = {
        lang: document.getElementById('s_lang').value,
        Setting: document.getElementById('Setting').value,
        Rules: document.getElementById('Rules').value,
        Nation: document.getElementById('Nation').value,
        Origin: document.getElementById('Origin').value,
    }
    // Объект сГенерированого Персонажа
    const GeneratedCharacter = {
        lang: CharacterSettings.lang,
        Setting: CharacterSettings.Setting,
        Rules: CharacterSettings.Rules,
    }
    ...
    if (CharacterSettings.Nation == 'Random') {
        GeneratedCharacter.Nation = NATIONS_7S[NATIONS_7S.list[getRandomInt(NATIONS_7S.list.length)]];
    // иначе выбрана конкретная — берём из таблицы название соотвктствующее сеттингу
    } else {
        GeneratedCharacter.Nation = NATIONS_7S[CharacterSettings.Nation];
    }
    // возвращаем на страницу в форму вывода сгенерированную Нацию
    document.getElementById('new_Nation').value = GeneratedCharacter.Nation;
    // генерируем Происходжение по процентной таблице
    GeneratedCharacter.Origin = selectFromTable(ORIGINS.England);
    GeneratedCharacter.Origin = NATIONS_7S[GeneratedCharacter.Origin];
    // возвращаем на страницу в форму вывода сгенерированнoe Происходжение
    document.getElementById('new_Origin').value = GeneratedCharacter.Origin;
    // GeneratedCharacter.Origin;
    // for (key in object) { /* тело цикла выполняется для каждого свойства объекта */ }
    GeneratedLog.push(toString(GeneratedCharacter));
...
function selectFromTable(OriginsTable = ORIGINS.England) {
  ...
  function Generate() { /* основной алгоритм генерации всего на свете */
    ...
    let Nation = NATIONS_7S[document.getElementById('Nation').value];
    let Origin = document.getElementById('Origin').value;
    let checkbox = document.getElementById('show_Origin');
    ...
    let ni; // НАЦИЯ //
    // этот блок пока необходим для работі старого алгоритма генерации
...
//    }
  ...
  // ИМЕНА //
    ...
    document.getElementById('NPC_Nation').value = Origin + Religion + ", " + Sexuality;
