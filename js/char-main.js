...
const Pnlty = []; // range attack penalty
...
const EnchDu = [[],[],[],[],[],]; // enchantment duration
    ...
    CHAR.attack.forEach( function createAttackBlock(item, index) {
        ...
        attackBlock.append(attack_Chars); // вставить "attack_Chars" в конец <`attackBlock_#`>
                ...
                aiming_Title_col.addEventListener("click", function addAimingBonus() {
                    if ((AttOpt[index] > -1) && (round_AP > 0)) {
                        if (AimAmn[index] < AimMax[index]) {
                            ...
                            attack_Roll.textContent = (ADice[index] === 'auto') ? "avto" :
                                ((AMod[index]-Pnlty[index]) >= 0) ? `${ADice[index]}D+`+`${AMod[index]-Pnlty[index]}=` :
                                 `${ADice[index]}D`+`${AMod[index]-Pnlty[index]}=`;
            ...
            if ("enchantments" in item) { // если в оружии есть Зачарование
                for (var i = 0; i < item.enchantments.length; i++) {
                    EnchaT[index][i] = false; // EnchaT[index][i]
                }
                ...
                attack_Chars.append(enchantments_Sel);
                    ...
                    enchantments_Sel.prepend(ench_Select);
                        ...
                        ench_0ption.selected = true;
                        ench_0ption.disabled = true;
                        ench_0ption.value = `0`;
                        ...
                        ench_Select.addEventListener("change", function fillEnchantment() {
                            ...
                            if (EnchOpt[index] > -1) { // если выбрано какое-то Зачарование
                                EnchaT[index][EnchOpt[index]] = true;
                                EnchDu[index][EnchOpt[index]] = item.enchantments[EnchOpt[index]].duration;
                                document.getElementById(`enchOption_${index+1}${EnchOpt[index]+1}`).disabled = true;
                                ...
                                if ("actionPoints" in item.enchantments[EnchOpt[index]]) EnchAP[index] =
                                    EnchAP[index] + item.enchantments[EnchOpt[index]].actionPoints;
                                else EnchAP[index] = EnchAP[index];
                                actionPoints.textContent = AttAP[index]-EnchAP[index];
                                // enchantments Attack Dice
                                if ("attackDice" in item.enchantments[EnchOpt[index]]) EnchAD[index] =
                                    EnchAD[index] + item.enchantments[EnchOpt[index]].attackDice;
                                else EnchAD[index] = EnchAD[index]; // enchantments Attack Modifinder
                                if ("attackMod" in item.enchantments[EnchOpt[index]]) EnchAM[index] =
                                    EnchAM[index] + item.enchantments[EnchOpt[index]].attackMod;
                                else EnchAM[index] = EnchAM[index];
                                attack_Roll.textContent = (ADice[index] === 'auto') ? "avto" :
                                    `${ADice[index]+EnchAD[index]}D+`+`${AMod[index]+EnchAM[index]}=`;
                                // enchantments Damage Dice
                                if ("damageDice" in item.enchantments[EnchOpt[index]]) EnchDD[index] =
                                    EnchDD[index] + item.enchantments[EnchOpt[index]].damageDice;
                                else EnchDD[index] = EnchDD[index]; // enchantments Damage Modifinder
                                if ("damageMod"  in item.enchantments[EnchOpt[index]]) EnchDM[index] =
                                    EnchDM[index] + item.enchantments[EnchOpt[index]].damageMod;
                                else EnchDM[index] = EnchDM[index];
            ...
            attack_Chars.append(kndn_Val);
                ...
                if (!item.complex && item.meel) { // если оружие простое ближнее / рукопашное
                    ...
                    if (item.reload) { // ++ ReLoad Action !!
                        ...
                        reload_option.selected = true;
                        reload_option.disabled = true;
                    ...
                    if (!item.complex && !item.wParts) { // Range Simple weappon
                        ...
                        ammoSelect.addEventListener("change", function fillAmmoValues() {
                            ...
                            //
                            attack_Roll.textContent = (ADice[index] === 'auto') ? "avto" :
                                ((AMod[index]+EnchAD[index]-Pnlty[index]) >= 0) ?
                                `${ADice[index]+EnchAD[index]}D+`+`${AMod[index]+EnchAD[index]-Pnlty[index]}=`
                                : `${ADice[index]+EnchAD[index]}D`+`${AMod[index]+EnchAD[index]-Pnlty[index]}=`;
                            // console.log(range / Rang[index]);
                            damage_Roll.textContent = `${DDice[index]+EnchDD[index]}D+`+`${DMod[index]+EnchDM[index]}=`;
                            criticalChance.textContent = Crit[index];
                            ...
                            document.getElementById('actionPointsResult').value = round_AP;
                            // document.getElementById(`attOption_${index+1}${o+1}`).disabled = true;
                            Load[index] = true;
        ...
        document.getElementById(`aIt_${index+1}`).parentNode.addEventListener("click", function attackByWeappon() {
            if (round_AP > 0) {
                if (AttAP[index] !== "–") {
                    let attack = (ADice[index] === 'auto') ? "avto" :
                        diceRoller((ADice[index]+EnchAD[index]), 6, (AMod[index]+EnchAM[index]-Pnlty[index]));
                    ...
                    if (!item.meel) {  // weapon range
                        ...
                        if ("ammunition" in item) {
                            ...
                            //
                            if (Ammo[index] === 0) {
                                document.getElementById(`attOption_${index+1}${AttOpt[index]+1}`).disabled = true;
                            ...
                            if ("ammunition" in item.wParts[AttOpt[index]]) {
                                ...
                                document.getElementById(`attOption_${index+1}${AttOpt[index]+1}`).textContent =
                                    `${item.wParts[AttOpt[index]].attackName} [${Ammos[index][AttOpt[index]]}]`;
                    ...
                    if (!item.meel && item.reload) {
                        document.getElementById(`attOption_${index+1}0reload`).selected = true;
                        Load[index] = false;
                        //
                        AttAP[index] = "–";
                        ADice[index] = "–";
                    ...
                    if ("enchantments" in item) {
                        if (EnchaT[index][EnchOpt[index]]) {
                            if (EnchDu[index][EnchOpt[index]] === 'hit') {
                                console.log(EnchDu[index][EnchOpt[index]], EnchaT[index][EnchOpt[index]]);
                                EnchaT[index][EnchOpt[index]] = false;
                                EnchAP[index] = ("actionPoints" in item.enchantments[EnchOpt[index]]) ?
                                EnchAP[index] - item.enchantments[EnchOpt[index]].actionPoints : EnchAP[index];
                                EnchAD[index] = ("attackDice" in item.enchantments[EnchOpt[index]]) ?
                                EnchAD[index] - item.enchantments[EnchOpt[index]].attackDice : EnchAD[index];
                                EnchAM[index] = ("attackMod" in item.enchantments[EnchOpt[index]]) ?
                                EnchAM[index] - item.enchantments[EnchOpt[index]].attackMod : EnchAM[index];
                                EnchDD[index] = ("damageDice" in item.enchantments[EnchOpt[index]]) ?
                                EnchDD[index] - item.enchantments[EnchOpt[index]].damageDice : EnchDD[index];
                                EnchDM[index] = ("damageMod"  in item.enchantments[EnchOpt[index]]) ?
                                EnchDM[index] - item.enchantments[EnchOpt[index]].damageMod : EnchDM[index];
                                document.getElementById(`enchOption_${index+1}0`).selected = true;
                                if (EnchAo[index][EnchOpt[index]] !== 0) {
                                    document.getElementById(`enchOption_${index+1}${EnchOpt[index]+1}`).disabled = false;
                    ...
                    //
                    actionPoints.textContent = (AttAP[index] === "–") ? "–" : AttAP[index]-EnchAP[index];
                    attack_Roll.textContent = (ADice[index] === 'auto') ? "avto" :
                        (AMod[index] === "–") ? `${ADice[index]}D${AMod[index]}` :
                        ((AMod[index]+EnchAM[index]-Pnlty[index]) >= 0) ?
                        `${ADice[index]+EnchAD[index]}D+`+`${AMod[index]+EnchAM[index]-Pnlty[index]}=` :
                        `${ADice[index]+EnchAD[index]}D`+`${AMod[index]+EnchAM[index]-Pnlty[index]}=`;
                    damage_Roll.textContent = (DMod[index] === "–") ? `${DDice[index]}D${DMod[index]}` :
                        `${DDice[index]+EnchDD[index]}D+${DMod[index]+EnchDM[index]}=`;
                    criticalChance.textContent = Crit[index];
                    criticalDamage.textContent = CrDam[index];
                    armorPiercing.textContent = ArmPi[index];
...
document.getElementById('end_fight').addEventListener( "click", () => {
    ...
    for (let i = 0; i < attackList.children.length; i++) { // проверяем и снимаем Зачарования с длительностью 'Весь Бой'
        let item = CHAR.attack[i];
        if ("enchantments" in item) {
            item.enchantments.forEach((jtem, j) => {
                if (EnchaT[i][j]) {
                    if (EnchDu[i][j] === 'fight') {
                        EnchaT[i][j] = false;
                        EnchAP[i] = ("actionPoints" in jtem) ? EnchAP[i] - jtem.actionPoints : EnchAP[i];
                        EnchAD[i] = ("attackDice" in jtem) ? EnchAD[i] - jtem.attackDice : EnchAD[i];
                        EnchAM[i] = ("attackMod" in jtem) ? EnchAM[i] - jtem.attackMod : EnchAM[i];
                        EnchDD[i] = ("damageDice" in jtem) ? EnchDD[i] - jtem.damageDice : EnchDD[i];
                        EnchDM[i] = ("damageMod"  in jtem) ? EnchDM[i] - jtem.damageMod : EnchDM[i];
                        document.getElementById(`actionPoints_a${i+1}`).textContent = AttAP[i]-EnchAP[i];
                        document.getElementById(`attack_${i+1}Roll`).textContent = (ADice[i] === 'auto') ? "avto" :
                            (AMod[i] === "–") ? `${ADice[i]}D${AMod[i]}` :
                            ((AMod[i]+EnchAM[i]-Pnlty[i]) >= 0) ?
                            `${ADice[i]+EnchAD[i]}D+`+`${AMod[i]+EnchAM[i]-Pnlty[i]}=` :
                            `${ADice[i]+EnchAD[i]}D`+`${AMod[i]+EnchAM[i]-Pnlty[i]}=`;
                        document.getElementById(`damage_${i+1}Roll`).textContent = (DMod[i] === "–") ?
                            `${DDice[i]}D${DMod[i]}` :
                            `${DDice[i]+EnchDD[i]}D+${DMod[i]+EnchDM[i]}=`;
                        document.getElementById(`enchOption_${i+1}0`).selected = true;
                        if (EnchAo[i][j] !== 0) {
                            document.getElementById(`enchOption_${i+1}${j+1}`).disabled = false;
                            // document.getElementById(`enchOption_${index+1}${EnchOpt[index]+1}`).remove();
                        }
                    }
                }
